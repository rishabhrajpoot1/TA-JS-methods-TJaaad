...
function nameWithS() {
  // your code goes here
  return got.houses.reduce((acc,cv) => {
    acc = acc.concat(cv.people.map((p)=>p.name).filter((name) => name.includes('s') || name.includes('S')
    ));
    return acc
  },[])
}
...
function nameWithA() {
  // your code goes here
  return got.houses.reduce((acc,cv) => {
    acc = acc.concat(cv.people.map((p)=>p.name).filter((name) => name.includes('a') || name.includes('A')
    ));
    return acc
  },[])
}

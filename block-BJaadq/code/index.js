...
function surnameWithS() {
  // your code goes here
  let allPeople = everyone();
  return allPeople.filter(name => name.split(" ")[1].startsWith('S'))
}

function surnameWithA() {
  // your code goes here
  let allPeople = everyone();
  return allPeople.filter(name => name.split(" ")[1].startsWith('A'))
}
...
let final = {};
got.houses.forEach((house)=>{
final[house.name] = house.people.map((p)=> p.name).join();
})
return final;
}
